...
def test_create_missing_sheet_row_valid():
    ...
    context = MissingSheetContext(Rule_Cd="?", Error_Category="Missing Sheet", Error_Severity_Cd="soft")
    sheet = "Sheet1"
...
def test_create_missing_sheet_row_invalid_sheet_type():
    ...
    with pytest.raises(ValueError, match="The 'sheet' argument must be a non-empty string."):
...
def test_create_missing_sheet_row_invalid_context_type():
    """Test the function when an invalid 'context' type is passed (not an instance of MissingSheetContext)."""
    with pytest.raises(ValueError, match="The 'context' argument must be of type MissingSheetContext."):
        create_missing_sheet_row("Sheet1", {})  # Passing a dictionary instead of a MissingSheetContext

def test_create_missing_sheet_row_invalid_rule_cd():
    """Test the function when an invalid 'Rule_Cd' is passed in the context."""
    invalid_context = MissingSheetContext(Rule_Cd="", Error_Category="Missing Sheet", Error_Severity_Cd="soft")
    with pytest.raises(ValueError, match="Invalid 'Rule_Cd' in context: it must be a non-empty string."):
...
def test_create_missing_sheet_row_invalid_error_category():
    ...
    invalid_context = MissingSheetContext(Rule_Cd="?", Error_Category="", Error_Severity_Cd="soft")
    with pytest.raises(ValueError, match="Invalid 'Error_Category' in context: it must be a non-empty string."):
...
def test_create_missing_sheet_row_invalid_error_severity_cd():
    ...
    invalid_context = MissingSheetContext(Rule_Cd="?", Error_Category="Missing Sheet", Error_Severity_Cd="")
    with pytest.raises(ValueError, match="Invalid 'Error_Severity_Cd' in context: it must be a non-empty string."):
        create_missing_sheet_row("Sheet1", invalid_context)  # Error_Severity_Cd is an empty string