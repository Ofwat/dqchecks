...
def test_create_dataframe_missing_sheets_invalid_sheet_name():
    ...
    with pytest.raises(ValueError, match="Invalid sheet name: ''. Each sheet must be a non-empty string."):
...
def test_create_dataframe_missing_sheets_no_missing_sheets():
    ...
    df = create_dataframe_missing_sheets(input_data, context)
    assert len(df) == 0  # The result should be an empty DataFrame since no missing sheets are provided
